    ...
    ) : IWordFileService
{
    public async Task<ResultDto<bool>> CreateWordFileAsync(int fileId, string username, CancellationToken ct)
    {
        ...
        using (var stream = openXmlWordService.CreateDocument(file.Title, file.Content)) 
        {
            ...
            var readyResult = await fileRepository.ReadyFileAsync(file, user, blobId, ct);

            if (!readyResult.Result || readyResult.ErrorCode != Error.None)
                return new ResultDto<bool>(false,
                    readyResult.ErrorCode == Error.None ? Error.InternalServerError : readyResult.ErrorCode,
                    readyResult.ErrorMessages);
